...
<head>
	...
	<title>Hi-Fi Cars | My Listings</title>
...
<main>
	<section class="container py-4 mt-5">
		<h1 class="display-6 text-center my-3">My Listings</h1>
					...
					<div class="col">
						<div class="card h-100">
							...
							<c:if test="${listing[0].status == 'Active'}">
								<span class="badge top-0 start-50 translate-middle text-bg-success position-absolute">${listing[0].status}</span>
							</c:if>
							...
							<c:if test="${listing[0].status == 'Inactive'}">
								<span class="badge top-0 start-50 translate-middle text-bg-danger position-absolute">${listing[0].status}</span>
							</c:if>
